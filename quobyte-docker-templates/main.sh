...
if [ ! -z $CLOUD_INSTALL ]; then
  ...
  if [ "$STRIPPEDNAME" == "quobyte-meta-" ];then
      for i in $(basename $(ls -d /var/lib/quobyte/devices/metadata*))
      do
  ...
  if [ -z $QUOBYTE_WEBCONSOLE_PORT ]; then
    QUOBYTE_WEBCONSOLE_PORT=8080
  else
   if [ $(echo $QUOBYTE_WEBCONSOLE_PORT | grep -c "^tcp") -eq 0 ]; then
     QUOBYTE_WEBCONSOLE_PORT=$(echo $QUOBYTE_WEBCONSOLE_PORT | awk -F: '{print $3}')
   fi 
  fi
...
if [ ! -z $CLOUD_INSTALL ]; then
  if [ "$STRIPPEDNAME" != "quobyte-reg-" ];then
    ...
    for i in $(seq 0 3)
    do
      reg=$(dig +short quobyte-reg-${i}.quobyte.${CLUSTER_NS}.svc.cluster.local)
      if [[ -z $reg ]]; then
         echo "Waiting for quobyte-reg-${i}.quobyte.${CLUSTER_NS}.svc.cluster.local"
         sleep 1
      else
        ((j++))
      fi
